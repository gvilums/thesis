...
static void read_input(T* A, unsigned int nr_elements) {
    srand(0);
    fprintf(stderr, "nr_elements\t%u\t", nr_elements);
...
int main(int argc, char **argv) {
    ...
    if (status) {
        fprintf(stderr, "[" ANSI_COLOR_GREEN "OK" ANSI_COLOR_RESET "] Outputs are equal\n");
    } else {
        fprintf(stderr, "[" ANSI_COLOR_RED "ERROR" ANSI_COLOR_RESET "] Outputs differ!\n");
    }
