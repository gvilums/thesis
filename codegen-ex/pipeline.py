...
def compute_size_info(config, lookup: TemplateLookup, code: CodegenOutput) -> SizeInformation:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(f"{tmpdir}/size_info.c", "w") as f:
            f.write(size_info_template.render(num_inputs=num_inputs, num_globals=num_globals, num_constants=num_constants))
...
def optimize_reduce_params(sizes: SizeInformation) -> ReduceConfigParams:
    ...
    input_buf_size = max(pow(2, math.ceil(math.log2(max(sizes.input_sizes)))), 256)
    reduction_var_count = (base_size - 11 * (sizes.stack_size + input_buf_size * num_inputs)) // sizes.output_size
